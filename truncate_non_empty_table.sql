IF  EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.truncate_non_empty_table'))
Begin
drop procedure dbo.truncate_non_empty_table
end
go
  ...
  -- Fetch and persist FKs             
                WHILE ( @i <= ( SELECT  MAX(ID)
                    ...
                    BEGIN
                        ...
                        SET @DropStatementTemp = REPLACE(REPLACE(@DropStatement,
                                                              '<tablename>',
                                                              @TableName),
                                                         '<constraintname>',
                                                         @ConstraintName);
                        SET @CreateStatementTemp = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(@CreateStatement,
                                                              '<tablename>',
                                                              @TableName),
                                                              '<column>',
                                                              @ColumnName),
                        ...
                        SET @TruncateStatementTemp = REPLACE(@TruncateStatement,
                                                             '<tablename>',
                                                             @TableName);
